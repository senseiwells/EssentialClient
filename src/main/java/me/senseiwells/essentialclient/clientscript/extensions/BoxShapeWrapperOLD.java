...
public class BoxShapeWrapperOLD implements IArucasWrappedClass {
	private static final Map<UUID, Set<BoxShapeWrapperOLD>> BOXES_TO_RENDER = new LinkedHashMap<>(0);
	...
	@ArucasFunction
	public NullValue setColor(Context context, NumberValue redValue, NumberValue greenValue, NumberValue blueValue) {
	...
	public synchronized static boolean addBoxToRender(Context context, BoxShapeWrapperOLD boxShapeWrapper) {
		Set<BoxShapeWrapperOLD> boxShapeWrapperSet = BOXES_TO_RENDER.computeIfAbsent(context.getContextId(), id -> {
	...
	public synchronized static boolean removeBoxToRender(Context context, BoxShapeWrapperOLD boxShapeWrapper) {
		Set<BoxShapeWrapperOLD> boxShapeWrapperSet = BOXES_TO_RENDER.get(context.getContextId());
		return boxShapeWrapperSet != null && boxShapeWrapperSet.remove(boxShapeWrapper);
	}

	public synchronized static List<BoxShapeWrapperOLD> getThroughBoxesToRender() {
		return BOXES_TO_RENDER.values().stream().flatMap(Collection::stream).filter(box -> box.renderThroughBlocks).toList();
	}

	public synchronized static List<BoxShapeWrapperOLD> getNormalBoxesToRender() {
		return BOXES_TO_RENDER.values().stream().flatMap(Collection::stream).filter(box -> !box.renderThroughBlocks).toList();
	}
}
