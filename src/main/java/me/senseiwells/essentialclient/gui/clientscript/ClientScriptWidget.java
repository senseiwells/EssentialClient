...
public class ClientScriptWidget extends ElementListWidget<ClientScriptWidget.ScriptListEntry> {
	...
	class ScriptListEntry extends AbstractListEntry<ScriptListEntry> {
		...
		@Override
		public void render(RenderContextWrapper wrapper, int index, int y, int x, int width, int height, int mouseX, int mouseY, boolean hovered, float tickDelta) {
			wrapper.drawTextWithShadow(this.client.textRenderer, Texts.literal(this.name), x - 50, y + height / 2 - 9 / 2, 16777215);
