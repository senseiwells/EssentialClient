...
public class PlayerDef extends CreatableDefinition<ClientPlayerEntity> {
	...
	private Void use(Arguments arguments) {
		...
		switch (action.toLowerCase()) {
			case "hold" -> ClientScriptUtils.holdKey(client.options.keyUse);
			case "stop" -> ClientScriptUtils.releaseKey(client.options.keyUse);
	...
	private Void attack(Arguments arguments) {
		...
		MinecraftClient client = EssentialUtils.getClient();
		switch (action.toLowerCase()) {
			case "hold" -> ClientScriptUtils.holdKey(client.options.keyAttack);
			case "stop" -> ClientScriptUtils.releaseKey(client.options.keyAttack);
	...
	private Void setWalking(Arguments arguments) {
		ClientScriptUtils.modifyKey(arguments.skip().nextPrimitive(BooleanDef.class), EssentialUtils.getClient().options.keyForward);
		return null;
	}
	...
	private Void setSneaking(Arguments arguments) {
		ClientScriptUtils.modifyKey(arguments.skip().nextPrimitive(BooleanDef.class), EssentialUtils.getClient().options.keySneak);
		return null;
	}
	...
	private Void logout(Arguments arguments) {
		...
		ClientScriptUtils.ensureMainThread("logout", arguments.getInterpreter(), () -> {
			EssentialUtils.getNetworkHandler().onDisconnected(Text.of(reason));
		});
		return null;
	}
	...
	private Void interactItem(Arguments arguments) {
		...
		ClientScriptUtils.ensureMainThread("interactItem", arguments.getInterpreter(), () -> {
			ClientPlayerEntity player = EssentialUtils.getPlayer();
			EssentialUtils.getInteractionManager().interactItem(player,player.world, hand);
		});
		return null;
	}
	...
	private void interactInternal(Interpreter interpreter, String directionAsString, Hand hand, Vec3d pos, BlockPos blockPos, boolean insideBlock) {
		...
		ClientScriptUtils.ensureMainThread("interactBlock", interpreter, () -> {
			ClientPlayerEntity player = EssentialUtils.getPlayer();
			EssentialUtils.getInteractionManager().interactBlock(player, player.clientWorld, hand, hitResult);
		});
	}
}
